...
namespace Master.Services.Data
{
    public class PriceRepository : IPriceRepository
    {
        ...
        public async Task<List<PriceObject>> GetPriceByProductIdAsync(PriceObjectInputObject input)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                return (await connection.QueryAsync<PriceObject>(sql, new { input.prod_id })).AsList();
            }
        }
        public async Task<OperationStatus> UpdatePriceByProductIdAsync(PriceUpdateObject input)
        {
            ...
            try
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    string sql = @"UPDATE prod_price_dtls SET price=@price,mod_by_usr_cd=@mod_by_usr_cd,mod_dttm=GETDATE() WHERE price_id=@price_id AND prod_id = @prod_id;";
